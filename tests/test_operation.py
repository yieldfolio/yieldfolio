...
def test_profitable_harvest(
    token_whale, chain, accounts, token, vault, strategy, user, strategist, amount, RELATIVE_APPROX
):
    # Deposit to the vault
    token.approve(vault.address, amount, {"from": user})
    vault.deposit(amount, {"from": user})
    ...
    token.transfer(strategy, token.balanceOf(token_whale)*0.02, {'from': token_whale})
