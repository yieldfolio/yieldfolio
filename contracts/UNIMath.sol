...
library UNIMath {
    ...
    /////////////////// ORACLE:
    function oracleAmountOut(uint32 _secondsAgo, uint256 _amountIn, address _tokenIn, address _tokenMid, address _tokenOut, uint24 _feeTokenToMid, uint24 _feeMidToOut) external view returns (uint256 _amountOut) {
        if (_tokenIn == _tokenOut || _amountIn == 0) {
            return _amountIn;
        }
        if (_tokenIn == _tokenMid || _tokenMid == _tokenOut ){
            require (_feeTokenToMid == _feeMidToOut, "feeTokenToMid != feeMidToOut");
            return _oracleAmountOutUNIV3(_secondsAgo, _amountIn, _tokenIn, _tokenOut, _feeTokenToMid);
        }
        return _oracleAmountOutUNIV3(_secondsAgo, _oracleAmountOutUNIV3(_secondsAgo, _amountIn, _tokenIn, _tokenMid, _feeTokenToMid), _tokenMid, _tokenOut, _feeMidToOut);
    }

    function _oracleAmountOutUNIV3(uint32 secondsAgo, uint256 _amountIn, address _tokenIn, address _tokenOut, uint24 _fee) internal view returns (uint256 _amountOut) {
        if (_tokenIn == _tokenOut || _amountIn == 0) {
            return _amountIn;
        }
        address pool = IUniswapV3Factory(univ3factory).getPool(_tokenIn, _tokenOut, _fee);
        require(pool != address(0), "pool doesn't exist!");
        //(int24 tick,) = IOracleLibrary.consult(pool, _secondsAgo);
        ...
        tick--;
        //calculate _amountOut from swap:
        _amountOut = UNIMath.getQuoteAtTick(tick, uint128(_amountIn), _tokenIn, _tokenOut);
    }
